...
class GetAssistant200Response(BaseModel):
    ...
    owner_slug: StrictStr = Field(
        description="Slug of the user or organization that owns the assistant", alias="ownerSlug"
    )
    ...
    icon_url: Optional[StrictStr] = Field(
        default=None, description="Pre-signed URL for the assistant's icon", alias="iconUrl"
    )
    on_prem_proxy_url: Optional[StrictStr] = Field(
        default=None, description="URL of the on-premises proxy if the organization uses one", alias="onPremProxyUrl"
    )
    use_on_prem_proxy: Optional[StrictBool] = Field(
        default=None, description="Whether the organization uses an on-premises proxy", alias="useOnPremProxy"
    )
    raw_yaml: Optional[StrictStr] = Field(
        default=None, description="Raw YAML configuration of the assistant", alias="rawYaml"
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "configResult": ListAssistants200ResponseInnerConfigResult.from_dict(obj["configResult"])
                if obj.get("configResult") is not None
                else None,
